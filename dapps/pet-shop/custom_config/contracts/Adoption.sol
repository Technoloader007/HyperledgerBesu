...
contract Blood{
    ...
    address payable public admin = address(0x7dDB03275288B2545F49d2E0A1A45D6B44461336);
    address payable public hospi = address(0x7dDB03275288B2545F49d2E0A1A45D6B44461336);
    ...
    mapping(address => userAccept) public donorD1;
    ...
    struct userAccept {
        ...
        uint256 hospital_number;
        string newuser_name;
        string date;
        uint256 newuserNumber;
    ...
    constructor() public{
        name = "Aman";
        userCount ++;
        usertype[admin] = User(userCount, 14, "Admin", admin, "Aman", '', '', "Jaipur");
        users[userCount] = User(userCount, 14, "Admin", admin, "Aman", '', '', "Jaipur");
    }
    ...
    function createBank(address payable _bank, string memory _name, string memory _city) public returns(string memory) {
    ...
    function useBag(
     uint bag_id,
     string memory h_name,
     uint256 h_number ,
     string memory newuser_name,
     uint256 newuser_number,
     string memory date,
     string memory newuser_email,
     string memory amount_blood,
     string memory city
      ) public payable {
        require(usertype[msg.sender].user_type == 3, 'Unauthorized transaction originator');
        ...
        donorD1[donor] = userAccept(h_name , h_number , newuser_name , date , newuser_number, newuser_email,amount_blood , city);
        emit userAccepted(h_name , h_number , newuser_name , date , newuser_number, newuser_email,amount_blood , city);
    }
